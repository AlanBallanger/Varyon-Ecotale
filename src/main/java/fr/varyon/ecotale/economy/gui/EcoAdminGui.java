package fr.varyon.ecotale.economy.gui;

import fr.varyon.ecotale.VaryonEcotalePlugin;
import fr.varyon.ecotale.economy.PlayerBalance;
import fr.varyon.ecotale.economy.TransactionEntry;
import fr.varyon.ecotale.economy.TransactionLogger;
...
public class EcoAdminGui extends InteractiveCustomUIPage<EcoAdminGui.AdminGuiData> {
    ...
    private String t(String key, String fallback) {
        return fr.varyon.ecotale.economy.util.TranslationHelper.t(playerRef, key, fallback);
    }
    ...
    public void handleDataEvent(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl Store<EntityStore> store, 
        ...
        if (!hasAdminPermission()) {
            fr.varyon.ecotale.economy.security.SecurityLogger logger = fr.varyon.ecotale.economy.security.SecurityLogger.getInstance();
            if (logger != null) {
        ...
        if (data.configSymbol != null && !data.configSymbol.isEmpty()) {
            ...
            if (data.configSymbol.length() <= 5 && isAsciiPrintable(data.configSymbol)) {
                VaryonEcotalePlugin.getInstance().getEconomyConfig().setCurrencySymbol(data.configSymbol);
            }
        ...
        if (data.configName != null) {
            ...
            if (data.configName.length() <= 20) {
                VaryonEcotalePlugin.getInstance().getEconomyConfig().setHudPrefix(data.configName);
            }
            refreshUI(ref, store);
            return;
        }
        ...
        if (data.configStarting != null && !data.configStarting.isEmpty()) {
            try {
                ...
                if (value >= 0 && value <= VaryonEcotalePlugin.getInstance().getEconomyConfig().getMaxBalance()) {
                    VaryonEcotalePlugin.getInstance().getEconomyConfig().setStartingBalance(value);
                }
        ...
        if (data.configMax != null && !data.configMax.isEmpty()) {
            try {
                ...
                if (value >= 1 && value <= 1e12) {
                    VaryonEcotalePlugin.getInstance().getEconomyConfig().setMaxBalance(value);
                }
            } catch (NumberFormatException ignored) {}
            refreshUI(ref, store);
            return;
        ...
        if (data.configDecimals != null && !data.configDecimals.isEmpty()) {
            try {
                ...
                if (value >= 0 && value <= 4) {
                    VaryonEcotalePlugin.getInstance().getEconomyConfig().setDecimalPlaces(value);
                }
            } catch (NumberFormatException ignored) {}
            refreshUI(ref, store);
            return;
        ...
        if (data.configFee != null && !data.configFee.isEmpty()) {
            try {
                ...
                if (value >= 0 && value <= 100) {
                    VaryonEcotalePlugin.getInstance().getEconomyConfig().setTransferFee(value / 100.0);
                }
            } catch (NumberFormatException ignored) {}
            refreshUI(ref, store);
            return;
        ...
        if (data.action != null) {
            ...
            switch (data.action) {
                    ...
                    if (selectedPlayerUuid != null) {
                        ...
                        if (amount >= 0) {  // -1 means invalid; 0 is valid for Set
                            ...
                            playerRef.sendMessage(Message.raw("Invalid amount (max: " + 
                                VaryonEcotalePlugin.getInstance().getEconomyConfig().format(VaryonEcotalePlugin.getInstance().getEconomyConfig().getMaxBalance()) + ")").color(Color.RED));
                        }
                    } else {
                ...
                case "SaveConfig" -> {
                    VaryonEcotalePlugin.getInstance().economyConfig.save();
    ...
    private void buildDashboard(@NonNullDecl UICommandBuilder cmd) {
        var allBalances = VaryonEcotalePlugin.getInstance().getEconomyManager().getAllBalances();
        ...
        cmd.set("#TotalCirculating.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(totalCirculating));
        cmd.set("#TotalPlayers.Text", String.valueOf(playerCount));
        cmd.set("#AverageBalance.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(average));
        
        // Config info
        cmd.set("#ConfigMaxBalance.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().formatShort(VaryonEcotalePlugin.getInstance().getEconomyConfig().getMaxBalance()));
        cmd.set("#ConfigTransferFee.Text", String.format("%.1f%%", VaryonEcotalePlugin.getInstance().getEconomyConfig().getTransferFee() * 100));
        cmd.set("#ConfigAutoSave.Text", (VaryonEcotalePlugin.getInstance().getEconomyConfig().getAutoSaveInterval() / 60) + " min");
    ...
    private void buildPlayersTab(@NonNullDecl UICommandBuilder cmd, @NonNullDecl UIEventBuilder events) {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            cmd.set("#PlayerList[" + displayIndex + "] #PlayerBalance.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(balance.getBalance()));
    ...
    private void buildTopTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        for (var entry : top10) {
            ...
            cmd.set("#TopList[" + (rank-1) + "] #PlayerBalance.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(balance.getBalance()));
    ...
    private void executeAction(String action, String uuidStr, String displayName, double amount, Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        switch (action) {
            case "Give" -> {
                ...
                lastFeedback = "+" + VaryonEcotalePlugin.getInstance().getEconomyConfig().formatShort(amount);
            }
            case "Take" -> {
                ...
                if (success) {
                    playerRef.sendMessage(Message.join(
                        ...
                        Message.raw(VaryonEcotalePlugin.getInstance().getEconomyConfig().format(amount)).color(new Color(255, 165, 0)),
            ...
            case "Set" -> {
                ...
                playerRef.sendMessage(Message.join(
                    ...
                    Message.raw(VaryonEcotalePlugin.getInstance().getEconomyConfig().format(amount)).color(new Color(255, 215, 0))
                ));
                lastFeedback = "=" + VaryonEcotalePlugin.getInstance().getEconomyConfig().formatShort(amount);
            }
        }
    ...
    private void executeReset(String uuidStr, String displayName, Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        playerRef.sendMessage(Message.join(
            ...
            Message.raw(VaryonEcotalePlugin.getInstance().getEconomyConfig().format(startingBalance)).color(Color.WHITE)
        ));
        lastFeedback = "RESET";
    }
    ...
    private double parseAmount(String input) {
        ...
        try {
            ...
            if (amount > VaryonEcotalePlugin.getInstance().getEconomyConfig().getMaxBalance()) {

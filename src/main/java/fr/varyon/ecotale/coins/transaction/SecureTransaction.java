package fr.varyon.ecotale.coins.transaction;
...
import static fr.varyon.ecotale.coins.util.TranslationHelper.t;
...
public class SecureTransaction {
    ...
    public static TransactionResult executeSecureExchange(
        ...
        try {
            ...
            EconomyBridge.deposit(playerUuid, (double) usedSourceValue, "TX_ESCROW:" + txHash);
            ...
            EconomyBridge.withdraw(playerUuid, (double) usedSourceValue, "TX_COMPLETE:" + txHash);
            ...
            if (!given) {
                ...
                EconomyBridge.deposit(playerUuid, (double) usedSourceValue, "TX_FALLBACK:" + txHash);
                updateTransactionStatus(txHash, "ROLLED_BACK_TO_BANK", 
    ...
    public static TransactionResult executeSecureWithdraw(
        ...
        try {
                        long bankBalance = fr.varyon.ecotale.coins.currency.BankManager.getBankBalance(playerUuid);
            ...
            boolean withdrawn = EconomyBridge.withdraw(playerUuid, (double) amount, "TX_WITHDRAW:" + txHash);
            if (!withdrawn) {
            ...
            if (!given) {
                ...
                EconomyBridge.deposit(playerUuid, (double) amount, "TX_ROLLBACK:" + txHash);
                updateTransactionStatus(txHash, "ROLLED_BACK_TO_BANK", 
    ...
    public static TransactionResult executeSecureDeposit(
        ...
        try {
            ...
            EconomyBridge.deposit(playerUuid, (double) actuallyTaken, "TX_DEPOSIT:" + txHash);
